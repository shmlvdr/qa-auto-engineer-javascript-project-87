...
describe('gendiff', () => {
  ...
  test('stylish format', () => {
    ...
    expect(genDiff(file1Yaml, file2Yaml, 'stylish')).toBe(readFile(getFixturePath('result_stylish.txt')));
  });
  ...
  test('plain format', () => {
    ...
    expect(actual).toBe(expected);
    expect(genDiff(file1Yaml, file2Yaml, 'plain')).toBe(readFile(getFixturePath('result_plain.txt')));
  });
  ...
  test('json format', () => {
    ...
    expect(actual).toBe(expected);
    expect(genDiff(file1Yaml, file2Yaml, 'json')).toBe(readFile(getFixturePath('result_json.txt')));
  });
