...
export default (filepath1, filepath2, format = 'stylish') => {
  ...
  const format1 = ext1 === '.json' ? 'json' : ['.yml', '.yaml'].includes(ext1) ? 'yml' : null;
  const format2 = ext2 === '.json' ? 'json' : ['.yml', '.yaml'].includes(ext2) ? 'yml' : null;

  if (!format1 || !format2) {
    throw new Error('Unsupported file format. Supported formats: .json, .yml, .yaml');
  }
